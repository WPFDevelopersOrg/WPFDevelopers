...
namespace WPFDevelopers.Assists
{
    ...
    internal class WindowOldConfig
    {
        ...
        public WindowChrome WindowChrome = null;
    }
    ...
    internal class WindowOSHelper
    {
        public static Version GetWindowOSVersion()
        {
            ...
            try
            {
                string str = regKey.GetValue("CurrentMajorVersionNumber")?.ToString();
    ...
    public class WindowAcrylicBlur : Freezable
    {
        ...
        private static Color _BackgtoundColor = Color.FromArgb(0x01, 0, 0, 0); //设置透明色 防止穿透
        ...
        // Using a DependencyProperty as the backing store for WindowAcrylicBlur.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty WindowAcrylicBlurProperty =
            DependencyProperty.RegisterAttached("WindowAcrylicBlur", typeof(WindowAcrylicBlur), typeof(WindowAcrylicBlur), new PropertyMetadata(default(WindowAcrylicBlur), OnWindowAcryBlurPropertyChangedCallBack));

        private static void OnWindowAcryBlurPropertyChangedCallBack(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
        ...
        private static readonly DependencyProperty WindowOldConfigProperty =
            DependencyProperty.RegisterAttached("WindowOldConfig", typeof(WindowOldConfig), typeof(WindowAcrylicBlur), new PropertyMetadata(default(WindowOldConfig)));
        ...
        public static readonly DependencyProperty BlurColorProperty =
            DependencyProperty.Register("BlurColor", typeof(Color), typeof(WindowAcrylicBlur), new PropertyMetadata(default(Color)));
        ...
        public static readonly DependencyProperty OpacityProperty =
            DependencyProperty.Register("Opacity", typeof(double), typeof(WindowAcrylicBlur), new PropertyMetadata(default(double)));
        ...
        private static bool EnableAcrylicBlur(Window window, Color color, double opacity, bool enable)
        {
            ...
            uint opacityIn = (uint)(255 * opacity);

            accent.AccentState = accentState;

            if (enable)
            {
                uint blurColor = (uint)(color.R << 0 | color.G << 8 | color.B << 16 | color.A << 24);
                uint blurColorIn = blurColor;
                if (opacityIn > 0)
        ...
        private static void Window_Initialized(object sender, EventArgs e)
        {
            ...
            WindowOldConfig config = new WindowOldConfig()
            {
